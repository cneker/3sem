...
int main() {
	...
	do {
		...
		for (int Prohod = 0; Prohod < number - num_input_neuron; Prohod++) {
			...
			for (int i = 0; i < num_hidden_neuron; i++) {
				for (int j = 0; j < num_input_neuron; j++)
					W[(i * 10) + j].Wes -= A * standart_y[Prohod + j] *

					(Sig[i].Wes * Error * Sig[i].znach * (1 - Sig[i].znach));
	...
	for (int Proh = 0; Proh < 15; Proh++) {
		...
		for (int i = 0; i < num_hidden_neuron; i++) {
			for (int j = 0; j < num_input_neuron; j++) {
				Sig[i].Sum += W[j].Wes * standart_y[number - num_input_neuron + Proh +
		...
		cout << a * cos(b * x) + c * sin(d * x) << "     "
			<< standart_y[number + Proh] << "    "
			<< a * cos(b * x) + c * sin(d * x) - standart_y[number + Proh] << endl;
	}
	return 0;
}